...
class StarchTrendAtTAndRHLineChartWidget extends StatefulWidget {
  ...
  StarchTrendAtTAndRHLineChartWidget({Key? key,required this.selectedVariety, required this.currentRS, required this.T }) : super(key: key);

  @override
  State<StarchTrendAtTAndRHLineChartWidget> createState() => _StarchTrendAtTAndRHLineChartWidgetState();
}

class _StarchTrendAtTAndRHLineChartWidgetState extends State<StarchTrendAtTAndRHLineChartWidget> {
  ...
  Future<List<FlSpot>> futureLineChartData(Array timeVec, List<double?> starch) async {
    List<FlSpot> lineChartData = [];
    for (var time in timeVec){
      var dataPoint = double.parse((await widget.selectedVariety.calculate_starch(time, widget.T, widget.currentRS)).toStringAsFixed(2));
      starch.add(dataPoint);
    }
    for(var i = 0; i< timeVec.length; i++)
      lineChartData.add( FlSpot(timeVec[i], starch[i] ?? 0));
