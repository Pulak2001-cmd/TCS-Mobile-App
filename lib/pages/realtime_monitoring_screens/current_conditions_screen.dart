...
class CurrentConditionsScreen extends StatefulWidget {
  ...
  @override
  State<CurrentConditionsScreen> createState() =>
      _CurrentConditionsScreenState();
}
...
class _CurrentConditionsScreenState extends State<CurrentConditionsScreen> {
  ...
  void getLatestTimestamp(DatabaseReference dataRef) {
    ...
    sensorDataSubscription = dataRef.onValue.listen((event) {
      Map<dynamic, dynamic> dataPoints =
      ...
      setState(() {
        ...
        timestamp = DateFormat('d MMM, yy hh:mm aaa').format(
  ...
  double calculateTimeDifference(DateTime currentDate, DateTime startDate) {
    return double.parse(
        (currentDate.difference(startDate).inHours / 24).toStringAsFixed(2));
  }
                            ...
                            builder: (context, snapshot) {
                              ...
                              WidgetsBinding.instance
                                ...
                                Provider.of<AlertProvider>(context,
                                        listen: false)
                                    .checkForAlerts(
                                        context, latestSensorReading);
                              });
                              double temperatureReading =
                                  latestSensorReading['temperature']
                                              .runtimeType ==
                                          int
                                      ? latestSensorReading['temperature']
                                          .toDouble()
                                      : latestSensorReading['temperature'] ?? 0;
                              double humidityReading =
                                  latestSensorReading['humidity'].runtimeType ==
                                          int
                                      ? latestSensorReading['humidity']
                                          .toDouble()
                                      : latestSensorReading['humidity'] ?? 0;
                              double ethyleneConcReading =
                                  latestSensorReading['etheleneConc']
                                              .runtimeType ==
                                          int
                                      ? latestSensorReading['etheleneConc']
                                          .toDouble()
                                      : latestSensorReading['etheleneConc'] ??
                                          0;
                              double co2ConcReading =
                                  latestSensorReading['CO2Conc'].runtimeType ==
                                          int
                                      ? latestSensorReading['CO2Conc']
                                          .toDouble()
                                      : latestSensorReading['CO2Conc'] ?? 0;
                              ...
                              return Column(
                                ...
                                children: [
                                  buildText(
                                      'Temperature: ${temperatureReading.toStringAsFixed(1)} °C'),
                                  buildText(
                                      'Relative Humidity: ${humidityReading.toStringAsFixed(1)} %'),
                                  buildText(
                                      'Ethylene Conc: ${ethyleneConcReading.toStringAsFixed(1)} ppm'),
                                  buildText(
                                      'CO2 Conc: ${co2ConcReading.toStringAsFixed(1)} ppm'),
                                ],
                              );
                            },
                          )),
                              ...
                              if (snapshot.hasData) {
                                ...
                                if (users.length > 0) {
                                  user = users.firstWhere((user) =>
                                      user.email == 'aryanpandey048@gmail.com');
                                }
                                double initialWeight =
                                    (user?.initialWeight?.toDouble())!;
                                double transpirationRate =
                                    PotatoData.getTranspirationRate(
                                        latestSensorReading['temperature'] ?? 0,
                                        latestSensorReading['humidity'] ?? 0);
                                double time = currentDashboard == 1
                                    ? calculateTimeDifference(
                                        currentDate, user!.startDate!)
                                    : calculateTimeDifference(
                                        currentDate, user!.startDate2!);
                                double currentWeight = PotatoData.getWeightloss(
                                    initialWeight, transpirationRate, time);
                                double currentWeightlossPercentage = PotatoData
                                    .calculateCurrentWeightlossPercentage(
                                        initialWeight, currentWeight);
                                double shelfLife =
                                    PotatoData.calculateRemainingShelflife(
                                ...
                                return Column(
                                  ...
                                  children: [
                                    ...
                                    //     SizedBox(),
                                    Provider.of<UselessProvider>(context)
                                            .isLotsHealthy
                                        ? buildTextAlert('Lots Healthy')
                                        : buildTextAlert('Lots Unhealthy'),
                                    buildText(
                                        'Shelf Life: ${shelfLife.toStringAsFixed(2)} days'),
                                    buildText(
                                        'Weight: ${currentWeight.toStringAsFixed(2)} Kgs'),
                                  ],
                                );
                                ...
                                if (snapshot.hasData) {
                                  ...
                                  return Column(
                                    ...
                                    children: [
                                      currentDashboard == 1
                                          ? buildText(
                                              'Start Date: ${DateFormat('dd-MM-yyyy').format(user.startDate!)}')
                                          : buildText(
                                              'Start Date: ${DateFormat('dd-MM-yyyy').format(user.startDate2!)}'),
                                      buildText(
                                          'Current Date: ${DateFormat('dd-MM-yyyy').format(currentDate)}'),
                                      currentDashboard == 1
                                          ? buildText(
                                              'Total Days: ${calculateTimeDifference(currentDate, user.startDate!)}')
                                          : buildText(
                                              'Total Days: ${calculateTimeDifference(currentDate, user.startDate2!)}'),
                                      buildText(
                                          'Initial weight: ${user.initialWeight} kgs'),
                                    ],
                                  );
                                } else if (snapshot.hasError) {
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              color:
                  text == 'Lots Unhealthy' ? HexColor('#FF5F6D') : Colors.white,
              fontSize: 20,
